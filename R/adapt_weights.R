...
# let each parameter vary but fix two to have a large penalty
"ns_adapt_fixed2" <- function(
	fuse, lambdas,
	y, S, X, starts,
	R, Rn, B, Bn,
	verbose=TRUE, all=FALSE, parallel=FALSE, gpu=FALSE
	...
	for (try in 1:3) {
		...
		for (iparam in 1:length(params)) {
			...
			for (ilambda in 1:length(lambdas)) {
				...
				try.weights <- rep( max(lambdas), 3 )
				try.weights[iparam]  <- 1
				try.weights[-iparam] <- try.weights[-iparam]/lambda

				# fit model and gather results
				tryCatch(expr={
					fit <- ns_estimate_all(lambda=lambda,y=y[-in.h,],X=X[-in.h,,,drop=FALSE],S=S[-in.h,],
						R=R[-in.h],Rn=Rn,B=B[-in.h],Bn=Bn,cov.params=cov.params,inits=inits,weights=try.weights,
						fuse=fuse,verbose=verbose,all=all,parallel=parallel)

					c_ll <- ns_cond_ll(X[-in.h,,,drop=FALSE], X[in.h,,,drop=FALSE], y[-in.h,], y[in.h,],
						fit$beta, fit$tau, fit$sigma, fit$phi, S[-in.h,], S[in.h,], R[-in.h], R[in.h], gpu=gpu)

					err <- rbind(err, data.frame(param=param, lambda=lambda, c_ll=c_ll))
				}, error=function(e) {
					err <<- rbind(err, data.frame(param=param, lambda=lambda, c_ll=best.c_ll))
				})
			...
			max.c_ll <- which.max(err$c_ll)
			perr <- rbind(perr, data.frame(param=param, lambda=err[max.c_ll,"lambda"], c_ll=err[max.c_ll,"c_ll"]) )
		}
